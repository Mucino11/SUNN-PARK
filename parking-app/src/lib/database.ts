...
// Modified to login with email or username
export const signIn = async (identifier: string, password: string) => {
  ...
  if (isEmail) {
    ...
    try {
      console.log(
        "Attempting login with username (case-insensitive):",
        identifier
      );
      ...
      console.log(
        `Found email '${userEmail}' for username '${identifier}'. Proceeding with email login.`
      );
